...
> Baseline snapshot only. Current implementation decisions and verification records live in [`implementation-ledger.md`](implementation-ledger.md).

Agent Harness v1 is RepoAgent Harness's current runtime shape: a local control loop around a model, repository context, constrained tools, task state, memory, and auditable run artifacts.
