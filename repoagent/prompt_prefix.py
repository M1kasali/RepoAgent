...
def build_prompt_prefix(workspace, tools, built_at=None, *, execution_context="", native_tools=False, state_root=None):
    ...
    text = textwrap.dedent(
        f"""\
        You are repoagent, a small local coding agent working inside a local repository.
    ...
    return PromptPrefix(
        text=text,
        hash=hashlib.sha256(
            (json.dumps([text, signature], ensure_ascii=True) if native_tools else text).encode("utf-8")
        ).hexdigest(),
