...
from .internal_tools import ToolRegistry
from .config import ContextConfig
from .base import AssemblyContext, Segment
from .curator import (CuratorArchiveMessagesTool, CuratorArchiveStore, CuratorAssembler, CuratorBuildContextTool, CuratorCheckBudgetTool, CuratorReadMemoryTool, CuratorRetrieveArchivedTool, CuratorSearchHistoryTool, CuratorSetRelevanceTool, CuratorState, CuratorUpdateWorkingStateTool, TurnContext, _curator_input_payload, _trace_messages)
from .persistence import MemoryStore
from .helpers import build_assistant_message
...
class CuratorSegmentBuilder:
    ...
    async def build(self, ctx: AssemblyContext) -> Segment | None:
        ...
        try:
            seg, fallback_meta = await asyncio.wait_for(self._slow_path(state, turn_id), timeout=self.config.curator_timeout_seconds)
            if seg is not None:
                return seg
    ...
    def _system_prompt() -> str:
        return """You are RepoAgent Curator, an internal context manager.
