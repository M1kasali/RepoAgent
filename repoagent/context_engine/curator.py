...
from .internal_tools import Tool
from .config import ContextConfig
from .base import AssembledPrefix
from .history_trimmer import HistoryTrimmer
from .values import AssembledContext, TokenBudget
from .helpers import ensure_dir, estimate_message_tokens, safe_filename
from .persistence import MemoryStore, sanitize_persisted_payload, sanitize_persisted_text
...
class CuratorArchiveStore:
    ...
    def retrieve(self, refs: list[str], max_tokens: int = 2000, mode: str = "snippet") -> dict[str, Any]:
        ...
        for ref in refs:
            ...
            if not path.resolve().is_relative_to(self.archive_dir.resolve()):
                results.append({"archive_ref": ref, "error": "outside archive", "messages": []})
                continue
            if not path.exists():
