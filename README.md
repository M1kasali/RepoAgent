...
当前 `feat/issue-maintainer-demo` 分支增加了面向维护者的 Issue 调查与候选修复 Demo：显式接入问题、隔离复现、人工启动修复、独立验证补丁。不自动发布 PR，使用方式和实测边界见 [Demo 说明](docs/issue-maintainer-demo.md)。
...
`run_tests` 支持 Python `unittest` 发现（`start`、`pattern`、`timeout`），
通过当前沙箱内的 `python3` 输出结构化测试结果，并绑定工作区文件指纹。
文件变化后，检查点会将旧结果标记为过期；零测试、全跳过或报告不完整不算通过。
普通 `run_shell` 的退出码不会自动成为测试验证结果。该记录不覆盖外部依赖状态，
也不能替代防止测试被削弱的独立评测。
